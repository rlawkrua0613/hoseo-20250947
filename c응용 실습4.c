...
void sale_max(int* count, int* sale, char name[][MAX]) {
    int max = sale[0];
    for (int i = 0; i < *count; i++) {
        if (sale[i] > max) {
            max = sale[i];
    ...
    for (int i = 0; i < *count; i++) {
        if (sale[i] == max) {
            printf("(id: %d) 상품명: %s ", i + 1, name[i]);
        }
    }
    printf("[판매량: %d]", max);
}

void sale_min(int* count, int* sale, char name[][MAX]) {
    int min = sale[0];
    for (int i = 0; i < *count; i++) {
        if (sale[i] < min) {
            min = sale[i];
...
void stock_warning(int* count, int* item, int* sale, char name[][MAX]) {
    for (int i = 0; i < *count; i++) {
        if (item[i] - sale[i] < 3) {
            printf("상품id %d, 상품명: %s 재고부족(%d)\n", i + 1, name[i], item[i] - sale[i]);
        }
    }

}

void item_name(int* count, char name[][MAX]) {
    for (int i = 0; i < *count; i++) {
        printf("id %d 상품명: ", i + 1);
        scanf("%s", &name[i]);
    }
...
int main() {
    ...
    while (1) {
        ...
        if (menu == 1) {
            ...
            stock_warning(&count, item, sale, name);
            printf("\n");

            total_sale(&count, sale, item);
            printf("\n");

            sale_max(&count, sale, name);
            printf("\n");

            sale_min(&count, sale, name);
            printf("\n");
